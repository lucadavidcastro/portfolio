...
def llm(prompt):
    ...
    try:
        ...
        log(f'LLM_FALLBACK {e}')
        return None

def fallback_bid(task):
    title=str(task.get('title','')).strip()
    blob=(title+' '+str(task.get('description',''))).lower()
    if 'name' in blob or 'naming' in blob:
        hook='I can deliver 3 distinct naming directions with concise rationale, ready for review within 24 hours.'
    elif any(x in blob for x in ['content','copy','script']):
        hook='I can deliver a concise, usable content/copy solution aligned to the brief within 24 hours.'
    elif any(x in blob for x in ['video','reel','tiktok','short-form']):
        hook='I can deliver a practical short-form concept/structure with clear execution notes within 24 hours.'
    else:
        hook='I can deliver a concise, buyer-ready result based strictly on the brief within 24 hours.'
...
def services(token,state):
    existing=set(state.get('v3_services',[]))
    specs=[
      ('Creative Naming Sprint','Three usable naming directions for a product, project, artist, campaign or brand brief. Fast 24-hour turnaround.',['naming','brand','creative','strategy'],2500),
      ('Content Hook Pack','Five hooks plus a simple content angle for a campaign, social series, artist release or short-form video brief.',['content','copy','social','creative'],1500),
      ('Short-form Concept Sprint','Three short-form video concepts with hook, structure and CTA for Reels, TikTok or Shorts.',['video','reels','tiktok','content'],2000),
      ('Music Release Creative Pack','Release concept, content angles and short-form ideas for a single, EP or album campaign.',['music','artist','campaign','content'],2000)
    ]
    for title,desc,tags,price in specs:
        if any(v.get('title')==title for v in state.get('v3_service_records',[])):continue
        ...
        log(f'SERVICE_V3 title={title} code={code}')
        if code in (200,201):
            sid=d.get('service',{}).get('id');state.setdefault('v3_services',[]).append(sid);state.setdefault('v3_service_records',[]).append({'title':title,'id':sid,'priceCents':price})

def target_price(budget):
    b=max(1000,int(budget or 0))
    return min(b,max(1000,int(b*0.55)))
...
def retarget(token,jid,agent_id,budget,state):
    ...
    for bid in d.get('bids',[]):
        ...
        if bid_id and current>desired and bid.get('status')=='PENDING':
            st,_=http('PATCH',f'/agents/jobs/{jid}/bids/{bid_id}',{'priceCents':desired,'message':fallback_bid({'title':state.get('v3_seen',{}).get(jid,{}).get('title','task')})},token)
            log(f'REPRICE {jid} code={st} price={desired}')
        return
...
def jobs(token,state,agent_id):
    ...
    for q in SEARCHES:
        ...
        if code!=200:
            log(f'JOBS_FAIL q={q} code={code}');continue
    ...
    for jid,p in discovered.items():
        ...
        if jid in state.setdefault('v3_seen',{}):
            retarget(token,jid,agent_id,budget,state)
            continue
...
def main():
    ...
    try:
        ...
        services(tok,state);jobs(tok,state,a.get('id'));deliveries(tok,state)
        log('V3_OK')
